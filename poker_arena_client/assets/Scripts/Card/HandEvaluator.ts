...
export enum HandType {
    ...
    BOMB = 12,          // Bomb (4 of a kind)
    STRAIGHT_BOMB = 13, // Straight flush (5+ consecutive same suit)
    JOKER_BOMB = 14     // Joker bomb (2 jokers)
}
...
export class HandEvaluator {
    ...
    private static evaluateJokerBomb(cards: number[], levelRank: number): HandResult {
        const suit1 = CardUtils.getSuit(cards[0]);
        const suit2 = CardUtils.getSuit(cards[1]);

        if (suit1 === CardSuit.JOKER && suit2 === CardSuit.JOKER) {
            return {
                type: HandType.JOKER_BOMB,
                mainCards: cards,
                kickerCards: [],
                weight: 10000 // Highest bomb
            };
        }
    ...
    private static evaluateBomb(cards: number[], levelRank: number): HandResult {
        ...
        if (points[0] === points[1] && points[1] === points[2] && points[2] === points[3]) {
            ...
            return {
                ...
                mainCards: cards,
                kickerCards: [],
                weight: weight + 5000 // Bomb base weight
            };
        }
