...
export class TheDecreeModeClient extends GameModeClientBase {
    ...
    public onEnter(): void {
        log.debug('[TheDecreeModeClient] Entering game mode');
        this.isActive = true;
        ...
        log.debug('[TheDecreeModeClient] Waiting for server events...');
    }

    public onExit(): void {
        log.debug('[TheDecreeModeClient] Exiting game mode');
        this.isActive = false;
        ...
        if (this.communityCardsNode && this.communityCardsNode.isValid) {
            log.debug('[TheDecreeModeClient] Clearing community cards node children');
    ...
    public cleanup(): void {
        log.debug('[TheDecreeModeClient] Cleaning up');
        this.clearShowdownTimer();
        this.clearPendingTimeouts();
        super.cleanup();
        this.unregisterAllNetworkEvents();
    ...
    private findModeSpecificNodes(): void {
        ...
        if (this.theDecreeContainerNode) {
            log.debug('[TheDecreeModeClient] Found TheDecree container node');
            ...
            if (this.theDecreeUIController) {
                log.debug('[TheDecreeModeClient] Found TheDecreeUIController');
            } else {
                log.warn('[TheDecreeModeClient] TheDecreeUIController component not found');
            }
        } else {
            log.warn('[TheDecreeModeClient] TheDecree container node not found');
        }

        // 查找公共牌节点
        this.communityCardsNode = this.findNodeByName(this.game.node, 'CommunityCardsNode');
        if (this.communityCardsNode) {
            log.debug('[TheDecreeModeClient] Found CommunityCardsNode');
        } else {
            log.warn('[TheDecreeModeClient] CommunityCardsNode not found');
        }
    ...
    private initDealingAnimation(): void {
        log.debug('[TheDecreeModeClient] Initializing dealing animation system...');

        const pokerFactory = PokerFactory.instance;
        if (!pokerFactory) {
            log.warn('[TheDecreeModeClient] PokerFactory not found, skipping dealing animation init');
            return;
        }
        ...
        if (!pokerSprites || !pokerPrefab) {
            log.warn('[TheDecreeModeClient] Poker resources not found');
            return;
        }
        ...
        if (this.theDecreeContainerNode) {
            ...
            if (!deckPileNode) {
                log.debug('[TheDecreeModeClient] Creating DeckPile node dynamically');
                ...
                log.debug('[TheDecreeModeClient] Found existing DeckPile node:', deckPileNode.name);
            }
            ...
            log.debug('[TheDecreeModeClient] DeckPile initialized');
        }
        ...
        if (this.theDecreeContainerNode) {
            ...
            log.debug('[TheDecreeModeClient] DealingAnimator initialized');
        }

        log.debug('[TheDecreeModeClient] Dealing animation system ready');
    }
    ...
    public showUI(): void {
        log.debug('[TheDecreeModeClient] Showing UI');
        ...
        if (this.theDecreeUIController && this.theDecreeUIController.autoPlaySwitch) {
            ...
            log.debug('[TheDecreeModeClient] Auto-play switch shown');
        }
    ...
    public hideUI(): void {
        log.debug('[TheDecreeModeClient] Hiding UI');
    ...
    protected setupNetworkEvents(): void {
        log.debug('[TheDecreeModeClient] ========== Setting up network events ==========');
        log.debug('[TheDecreeModeClient] Registering event handlers...');
        ...
        log.debug('[TheDecreeModeClient] ✓ All network events registered');
        log.debug('[TheDecreeModeClient] ========================================');
    }
    ...
    private onGameReconnected = () => {
        log.debug('[TheDecreeModeClient] Game reconnected, resyncing UI/state...');
        log.debug('[TheDecreeModeClient] game instance:', {
        ...
        log.debug('[TheDecreeModeClient] playerUIManager exists:', !!this.game.playerUIManager);
        log.debug('[TheDecreeModeClient] current room:', !!LocalRoomStore.getInstance().getCurrentRoom());
    ...
    private onDealCards(data: DealCardsEvent): void {
        log.debug('[TheDecreeModeClient] ========== Deal Cards Event ==========');
        log.debug('[TheDecreeModeClient] 📩 Received deal_cards event from server');
        log.debug('[TheDecreeModeClient] Player ID:', data.playerId);
        log.debug('[TheDecreeModeClient] Cards:', data.handCards);
        log.debug('[TheDecreeModeClient] Card count:', data.handCards.length);
        log.debug('[TheDecreeModeClient] All hand counts:', data.allHandCounts);
        log.debug('[TheDecreeModeClient] Raw event data:', JSON.stringify(data));

        // 如果公共牌还没发完，先缓存玩家发牌事件
        if (!this._communityCardsDealt) {
            log.debug('[TheDecreeModeClient] Community cards not dealt yet, queuing player deal...');
    ...
    private processPlayerDeal(data: DealCardsEvent): void {
        ...
        if (!this.playerIdToIndexMap.has(data.playerId)) {
            log.warn(`[TheDecreeModeClient] Player ${data.playerId} not in mapping, reinitializing...`);
            log.debug('[TheDecreeModeClient] Current mapping before reinit:', Array.from(this.playerIdToIndexMap.entries()));
            this.upgradePlayerUIToPlayingMode();
            log.debug('[TheDecreeModeClient] Current mapping after reinit:', Array.from(this.playerIdToIndexMap.entries()));

            // 再次检查是否成功添加
            if (!this.playerIdToIndexMap.has(data.playerId)) {
                log.error(`[TheDecreeModeClient] Failed to add player ${data.playerId} to mapping after reinit!`);
                log.error('[TheDecreeModeClient] This means the player is not in LocalRoomStore');
                return;
            }
        }
        ...
        const playerUIManager = this.game.playerUIManager;
        if (!playerUIManager) {
            log.error('[TheDecreeModeClient] PlayerUIManager not found!');
            return;
        }

        // 获取玩家相对索引
        const playerIndex = this.getPlayerIndex(data.playerId);
        log.debug(`[TheDecreeModeClient] Player index (relative): ${playerIndex}`);

        if (playerIndex === -1) {
            log.warn(`[TheDecreeModeClient] Player ${data.playerId} not found in player list`);
            return;
        }
        ...
        if (playerUIController) {
            ...
            if (player) {
                log.debug(`[TheDecreeModeClient] Found player: ${player.name}`);
                ...
                log.debug(`[TheDecreeModeClient] Before update - handCards: ${oldHandCount}`);
                log.debug(`[TheDecreeModeClient] New hand count: ${newHandCount}, Actual new cards: ${actualDealCount}`);
                ...
                if (this.dealingAnimator && this.deckPile && actualDealCount > 0) {
                    log.debug(`[TheDecreeModeClient] Playing deal animation for ${actualDealCount} cards (isInitialDeal: ${isInitialDeal})...`);
                    this.deckPile.show();
                    ...
                    if (isMainPlayer) {
                        ...
                        this.dealingAnimator.dealToOtherPlayer(
                            ...
                            actualDealCount,
                            () => {
                                log.debug('[TheDecreeModeClient] Other player deal animation complete');
                    ...
                    if (actualDealCount <= 0) {
                        log.debug(`[TheDecreeModeClient] No new cards to deal (actualDealCount: ${actualDealCount}), updating display directly`);
                    } else {
                        log.debug(`[TheDecreeModeClient] Calling updatePlayerHand (no animation)...`);
                    }
                    playerUIManager.updatePlayerHand(playerIndex);

                    log.debug(`[TheDecreeModeClient] ✓ Hand display updated successfully`);

                    // 如果是主玩家（index 0），启用卡牌选择功能（允许预选）
                    if (isMainPlayer) {
                        log.debug('[TheDecreeModeClient] Enabling card selection for player 0 (pre-selection allowed)...');

                        // 通过 TheDecreeUIController 启用选牌（这样会正确更新 _selectedCardIndices）
                        if (this.theDecreeUIController) {
                            this.theDecreeUIController.enableCardSelection();
                            log.debug('[TheDecreeModeClient] ✓ Card selection enabled via UI controller (pre-selection mode)');
                        } else {
                            log.warn('[TheDecreeModeClient] TheDecreeUIController not found, cannot enable card selection');
                        }
                    }
                }
            } else {
                log.error(`[TheDecreeModeClient] ✗ Player not found in PlayerUIController for index ${playerIndex}`);
            }
        } else {
            log.error(`[TheDecreeModeClient] ✗ PlayerUIController not found for index ${playerIndex}`);
        }
        ...
        if (playerIndex === 0) {
            ...
            log.debug(`[TheDecreeModeClient] Updating other players hand displays (${playerCount - 1} players)...`);
            ...
            for (let i = 1; i < playerCount; i++) {
                ...
                if (otherPlayerUIController) {
                    ...
                    if (otherPlayer) {
                        ...
                        log.debug(`[TheDecreeModeClient] Player ${otherPlayer.name} at index ${i}: old=${oldOtherHandCount}, new=${newOtherHandCount}, deal=${otherDealCount}`);
                        ...
                        if (this.dealingAnimator && this.deckPile && otherDealCount > 0) {
                            ...
                            this.dealingAnimator.dealToOtherPlayer(
                                ...
                                otherDealCount,
                                () => {
                                    log.debug(`[TheDecreeModeClient] Other player ${i} deal animation complete`);
                        ...
                        log.debug(`[TheDecreeModeClient] ✓ Updated hand count for player at index ${i}`);
                    }
                }
            }
            log.debug('[TheDecreeModeClient] ✓ All other players hand counts updated');
        } else if (data.allHandCounts) {
            // 非主玩家的 deal_cards 事件（补牌场景），更新其他玩家
            log.debug('[TheDecreeModeClient] Updating all players hand counts from allHandCounts...');
            for (const [playerId, handCount] of Object.entries(data.allHandCounts)) {
                ...
                if (otherPlayerIndex === -1) {
                    log.warn(`[TheDecreeModeClient] Player ${playerId} not found in mapping`);
                    continue;
                }
                ...
                if (otherPlayerUIController) {
                    const otherPlayer = otherPlayerUIController.getPlayer();
                    if (otherPlayer) {
                        log.debug(`[TheDecreeModeClient] Updating hand count for player ${otherPlayer.name}: ${handCount} cards`);
                        ...
                        log.debug(`[TheDecreeModeClient] ✓ Updated hand count for player at index ${otherPlayerIndex}`);
                    }
                }
            }
            log.debug('[TheDecreeModeClient] ✓ All players hand counts updated');
        }

        // 更新牌堆显示数量
        if (data.deckSize !== undefined && this.deckPile) {
            log.debug(`[TheDecreeModeClient] Updating deck pile display: ${data.deckSize} cards remaining`);
    ...
    private async playMainPlayerDealAnimation(
        ...
        log.debug(`[TheDecreeModeClient] Playing main player deal animation for ${newCards.length} new cards (Plan C)...`);

        // 阶段1-3：堆叠 → 展开 → 翻牌（只对新发的牌）
        const result = await this.dealingAnimator.dealToMainPlayer(
            newCards,
            () => log.debug('[TheDecreeModeClient] Stack complete'),
            () => log.debug('[TheDecreeModeClient] Spread complete'),
            () => log.debug('[TheDecreeModeClient] Flip complete')
        );

        // 阶段4：排序动画
        if (result.cardNodes.length > 0) {
            log.debug('[TheDecreeModeClient] Starting sort animation...');

            // 获取排序后的所有牌
            const sortedAllCards = PlayerHandDisplay.sortCards(allCards);
            log.debug('[TheDecreeModeClient] Sorted all cards:', sortedAllCards.map(c => '0x' + c.toString(16)));
            ...
            if (handDisplay) {
                ...
                await this.dealingAnimator.animateSorting(
                    ...
                    sortedNewCards,
                    newCardPositions,
                    () => log.debug('[TheDecreeModeClient] Sort animation complete')
                );
            }
        ...
        if (this.theDecreeUIController) {
            this.theDecreeUIController.enableCardSelection();
            log.debug('[TheDecreeModeClient] ✓ Card selection enabled via UI controller');
        }

        log.debug('[TheDecreeModeClient] Main player deal animation complete');
    ...
    private async playMainPlayerRefillAnimation(
        ...
        log.debug(`[TheDecreeModeClient] Playing main player refill animation for ${newCards.length} new cards...`);

        // 获取手牌显示组件
        const handDisplay = playerUIController.getHandDisplay();
        if (!handDisplay) {
            log.error('[TheDecreeModeClient] HandDisplay not found');
            playerUIManager.updatePlayerHand(0);
            return;
        }
        ...
        const player = playerUIController.getPlayer();
        if (!player) {
            log.error('[TheDecreeModeClient] Player not found');
            playerUIManager.updatePlayerHand(0);
            return;
        }
        ...
        log.debug(`[TheDecreeModeClient] Displayed cards: ${displayedCards.map((c: number) => '0x' + c.toString(16)).join(',')}`);
        log.debug(`[TheDecreeModeClient] All cards (new hand): ${allCards.map(c => '0x' + c.toString(16)).join(',')}`);
        log.debug(`[TheDecreeModeClient] Played cards: ${playedCards.map((c: number) => '0x' + c.toString(16)).join(',')}`);
        log.debug(`[TheDecreeModeClient] Remaining: ${remainingCards.length}, New cards: ${newCards.length}`);

        // 步骤1：隐藏打出的牌（不重新布局，剩余牌保持原位置）
        const hiddenPositions = handDisplay.hideCardsWithoutRelayout(playedCards);
        log.debug(`[TheDecreeModeClient] Hidden ${hiddenPositions.length} played cards, positions preserved`);
        ...
        log.debug(`[TheDecreeModeClient] Target positions for new cards: ${targetPositions.length}`);

        // 步骤3：播放补牌动画（飞到空位置，然后翻牌）
        await this.dealingAnimator.refillToMainPlayer(
            newCards,
            targetPositions,
            () => log.debug('[TheDecreeModeClient] Refill flight animation complete')
        );
        ...
        // 步骤5：播放排序动画
        const sortedAllCards = PlayerHandDisplay.sortCards(allCards);
        log.debug('[TheDecreeModeClient] Playing sort animation...');
        ...
        log.debug('[TheDecreeModeClient] Main player refill animation complete');
    }
    ...
    private processPendingPlayerDeals(): void {
        if (this._pendingPlayerDeals.length === 0) {
            log.debug('[TheDecreeModeClient] No pending player deals to process');
            return;
        }

        log.debug(`[TheDecreeModeClient] Processing ${this._pendingPlayerDeals.length} pending player deals...`);
        ...
        // 清空队列
        this._pendingPlayerDeals = [];
        log.debug('[TheDecreeModeClient] All pending player deals processed');
    }

    private onCommunityCards(data: CommunityCardsEvent): void {
        log.debug('[TheDecreeModeClient] ========== Community Cards Event ==========');
        log.debug('[TheDecreeModeClient] 📩 Received community_cards event from server');
        log.debug('[TheDecreeModeClient] Cards:', data.cards);
        log.debug('[TheDecreeModeClient] Card count:', data.cards.length);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        log.debug('[TheDecreeModeClient] Raw event data:', JSON.stringify(data));
        ...
        if (this.dealingAnimator && this.deckPile) {
            log.debug('[TheDecreeModeClient] Playing community cards deal animation (all face-down, then flip)...');
            this.deckPile.show();
            ...
            this.dealingAnimator.dealCommunityCards(
                ...
                (index: number, cardValue: number) => {
                    log.debug(`[TheDecreeModeClient] Community card ${index + 1} dealt (face-down): 0x${cardValue.toString(16)}`);
                    this.displaySingleCommunityCard(index, cardValue, false); // 显示背面
                },
                // 全部发完后的回调 - 一起翻牌，然后处理待发的玩家牌
                () => {
                    log.debug('[TheDecreeModeClient] All community cards dealt, flipping together...');
            ...
            // 无动画，直接显示
            log.debug('[TheDecreeModeClient] Calling displayCommunityCards() (no animation)...');
            this.displayCommunityCards();
            log.debug('[TheDecreeModeClient] ✓ Community cards displayed');
    ...
    private onRequestFirstDealerSelection(data: RequestFirstDealerSelectionEvent): void {
        log.debug('[TheDecreeModeClient] ========== Request First Dealer Selection Event ==========');
        log.debug('[TheDecreeModeClient] 📩 Received request_first_dealer_selection event');
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        log.debug('[TheDecreeModeClient] 💡 提示：请选择一张手牌，牌最大的成为首个庄家');
        ...
        if (this.theDecreeUIController) {
            this.theDecreeUIController.enableCardSelection();
            log.debug('[TheDecreeModeClient] ✓ Card selection enabled for first dealer selection');
        }
    ...
    private onPlayerSelectedCard(data: PlayerSelectedCardEvent): void {
        log.debug('[TheDecreeModeClient] ========== Player Selected Card Event ==========');
        log.debug('[TheDecreeModeClient] 📩 Player selected a card:', data.playerId);

        // 显示其他玩家已经选择的状态
        const playerName = this.getPlayerName(data.playerId);
        log.debug(`[TheDecreeModeClient] 👤 ${playerName} 已选择`);

        log.debug('[TheDecreeModeClient] =====================================');
    }

    private onFirstDealerReveal(data: FirstDealerRevealEvent): void {
        log.debug('[TheDecreeModeClient] ========== First Dealer Reveal Event ==========');
        log.debug('[TheDecreeModeClient] 📩 Revealing first dealer selection');
        log.debug('[TheDecreeModeClient] Dealer ID:', data.dealerId);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        log.debug('[TheDecreeModeClient] All selections:', data.selections);

        // 构建显示信息
        log.debug('[TheDecreeModeClient] 🎴 所有玩家的选择：');
        ...
        for (const selection of data.selections) {
            ...
            log.debug(`[TheDecreeModeClient]   ${isDealer ? '👑' : '  '} ${playerName}: ${cardName}`);
        }

        // 找到dealer的名字
        const dealerName = this.getPlayerName(data.dealerId);
        log.debug(`[TheDecreeModeClient] 🎉 ${dealerName} 成为首个庄家！`);
        ...
        this.registerTimeout(() => {
            ...
            if (currentPlayerId === data.dealerId) {
                log.debug('[TheDecreeModeClient] 3 seconds passed, showing dealer call buttons...');
                if (this.theDecreeUIController) {
                    this.theDecreeUIController.updateCallButtonsVisibility();
                    log.debug('[TheDecreeModeClient] ✓ Dealer call buttons shown');
                }
            }
        }, 3000);
    ...
    private displayFirstDealerSelections(selections: { playerId: string; card: number }[], dealerId: string): void {
        log.debug('[TheDecreeModeClient] Displaying first dealer selections in each player\'s hand area');

        const playerUIManager = this.game.playerUIManager;
        if (!playerUIManager) {
            log.warn('[TheDecreeModeClient] PlayerUIManager not found');
            return;
        }
        ...
        for (const selection of selections) {
            ...
            if (playerIndex === -1) {
                log.warn(`[TheDecreeModeClient] Player ${selection.playerId} not found in mapping`);
                continue;
            }

            const playerUINode = playerUIManager.getPlayerUINode(playerIndex);
            if (!playerUINode) {
                log.warn(`[TheDecreeModeClient] PlayerUINode not found for index ${playerIndex}`);
                continue;
            }

            const handDisplay = playerUINode.getHandDisplay();
            if (!handDisplay) {
                log.warn(`[TheDecreeModeClient] HandDisplay not found for index ${playerIndex}`);
                continue;
            }
            ...
            if (playerIndex === 0) {
                log.debug(`[TheDecreeModeClient] Skipping updateDisplay for main player (index 0) - card already displayed with highlight`);
            } else {
            ...
            log.debug(`[TheDecreeModeClient] Displayed selection for ${playerName}: ${this.getCardName(selection.card)}${isDealer ? ' (Dealer 👑)' : ''}`);
        }

        log.debug('[TheDecreeModeClient] ✓ All selections displayed in hand areas');
    }
    ...
    private hideFirstDealerSelections(): void {
        log.debug('[TheDecreeModeClient] Hiding first dealer selections');
        ...
        if (this.theDecreeUIController) {
            this.theDecreeUIController.enableCardSelection();
            log.debug('[TheDecreeModeClient] ✓ Card selection re-enabled for main player');
        }

        log.debug('[TheDecreeModeClient] First dealer selections hidden');
    }
    ...
    private initializeAllPlayersHands(): void {
        log.debug('[TheDecreeModeClient] Initializing all players hands...');
        ...
        if (!currentRoom) {
            log.error('[TheDecreeModeClient] No current room found!');
            return;
        }
        ...
        for (let i = 0; i < currentRoom.players.length; i++) {
            ...
            if (playerUIController) {
                ...
                if (player && player.handCards.length === 0) {
                    ...
                    log.debug(`[TheDecreeModeClient] Initializing ${this.config.initialHandSize} cards for player ${i} (${playerInfo.name})`);
        ...
        log.debug('[TheDecreeModeClient] ✓ All players hands initialized');
    }

    private onDealerSelected(data: DealerSelectedEvent): void {
        log.debug('[TheDecreeModeClient] ========== Dealer Selected Event ==========');
        log.debug('[TheDecreeModeClient] Dealer selected:', data);
        log.debug('[TheDecreeModeClient] Dealer ID:', data.dealerId);
        log.debug('[TheDecreeModeClient] Round number:', data.roundNumber);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        ...
        this.cardsToPlay = 0;
        log.debug('[TheDecreeModeClient] Reset cardsToPlay to 0 for new round');

        // 更新 UI 状态，禁用出牌按钮
        if (this.theDecreeUIController) {
            this.theDecreeUIController.updateUIState();
            log.debug('[TheDecreeModeClient] UI state updated - play button should be disabled');
        }
        ...
        if (dealerIndex !== -1 && this.game.playerUIManager) {
            ...
            log.debug(`[TheDecreeModeClient] Round ${this.currentRoundNumber}, dealer: ${this.dealerId} (index: ${dealerIndex})`);
        }

        // 如果是第一回合（首庄选择），不立即显示 call 按钮
        // call 按钮会在 onFirstDealerReveal 的 3 秒延迟后显示
        if (data.roundNumber === 1) {
            log.debug('[TheDecreeModeClient] First round - dealer call buttons will be shown after first dealer reveal (3s delay)');
            log.debug('[TheDecreeModeClient] =====================================');
            return;
        }
        ...
        log.debug('[TheDecreeModeClient] Current player ID:', currentPlayerId);
        log.debug('[TheDecreeModeClient] Is current player dealer?', currentPlayerId === data.dealerId);

        if (currentPlayerId === data.dealerId) {
            log.debug('[TheDecreeModeClient] Current player is dealer, showing call buttons...');
            if (this.theDecreeUIController) {
                this.theDecreeUIController.updateCallButtonsVisibility();
                log.debug('[TheDecreeModeClient] ✓ Call buttons visibility updated');
            } else {
                log.error('[TheDecreeModeClient] ✗ TheDecreeUIController not found, cannot show call buttons');
            }
        } else {
            log.debug('[TheDecreeModeClient] Current player is NOT dealer, hiding call buttons');
        }
    ...
    private onDealerCalled(data: DealerCalledEvent): void {
        log.debug('[TheDecreeModeClient] ========== Dealer Called Event ==========');
        log.debug('[TheDecreeModeClient] Dealer ID:', data.dealerId);
        log.debug('[TheDecreeModeClient] Cards to play:', data.cardsToPlay);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        ...
        log.debug('[TheDecreeModeClient] Is current player the dealer?', isDealer);
        ...
        if (this.theDecreeUIController) {
            ...
            log.debug(`[TheDecreeModeClient] Showing message: "${message}"`);
        }

        // 卡牌选择功能已经在 onDealCards 时启用了
        // 这里只需要更新 UI 状态（启用出牌按钮等）
        log.debug(`[TheDecreeModeClient] Dealer called ${data.cardsToPlay} cards, updating UI state...`);

        if (this.theDecreeUIController) {
            // 如果是非dealer玩家，隐藏call按钮（因为dealer已经叫牌了）
            if (!isDealer) {
                log.debug('[TheDecreeModeClient] Non-dealer: hiding call buttons');
                this.theDecreeUIController.updateCallButtonsVisibility();
            }

            // 更新 UI 状态（这会根据选牌数量启用/禁用出牌按钮）
            this.theDecreeUIController.updateUIState();
            log.debug('[TheDecreeModeClient] ✓ UI state updated');

            // 显示出牌数量提示（持久显示，直到摊牌）
            this.theDecreeUIController.showCardsToPlayHint(data.cardsToPlay);
            log.debug('[TheDecreeModeClient] Showing cards-to-play hint');
        } else {
            log.warn('[TheDecreeModeClient] TheDecreeUIController not found');
        }
    ...
    private onPlayerPlayed(data: PlayerPlayedEvent): void {
        log.debug('[TheDecreeModeClient] Player played:', data);
    ...
    private onShowdown(data: ShowdownEvent): void {
        log.debug('[TheDecreeModeClient] ========== Showdown Event ==========');
        log.debug('[TheDecreeModeClient] Showdown results:', data);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);

        // 标记摊牌动画开始
        this._isShowdownInProgress = true;
        log.debug('[TheDecreeModeClient] Showdown animation started');
        ...
        if (this.theDecreeUIController) {
            ...
            log.debug('[TheDecreeModeClient] Hiding cards-to-play hint at showdown');
        }

        // 显示所有玩家的牌型和结果（日志）
        for (const result of data.results) {
            log.debug(`[TheDecreeModeClient] Player ${result.playerId}: ${result.handTypeName} (${result.score} points)${result.isWinner ? ' WINNER!' : ''}`);
            log.debug(`[TheDecreeModeClient]   Cards:`, result.cards);
            log.debug(`[TheDecreeModeClient]   Hand Type:`, result.handType, '-', result.handTypeName);
        }
        ...
        log.debug('[TheDecreeModeClient] Updating showdown display for all players');

        // 先更新所有玩家的手牌显示（立即显示出的牌）
        for (const result of data.results) {
            log.debug(`[TheDecreeModeClient] Processing result for player ${result.playerId}`);
            log.debug(`[TheDecreeModeClient]   Cards (hex):`, result.cards.map(c => '0x' + c.toString(16)));

            // 使用 getPlayerIndex 获取相对索引（重要！）
            const playerIndex = this.getPlayerIndex(result.playerId);
            if (playerIndex === -1) {
                log.warn(`[TheDecreeModeClient] Player ${result.playerId} not found in player mapping`);
                continue;
            }

            log.debug(`[TheDecreeModeClient] Showing cards for player ${result.playerId} at relative index ${playerIndex}`);
            ...
            if (isMe) {
                ...
                if (isAuto) {
                    ...
                    log.debug(`[TheDecreeModeClient] Showing my auto-played cards:`, result.cards);
                    ...
                    log.debug(`[TheDecreeModeClient] Auto-played card indices:`, indicesToSelect);
                    ...
                    // 手动出牌：牌已经高亮显示了，不需要更新
                    log.debug(`[TheDecreeModeClient] Skipping updateDisplay for my manual play - cards already highlighted`);
                }
            } else {
                // 其他玩家的牌：显示正面
                handDisplay.updateDisplay(result.cards);
                log.debug(`[TheDecreeModeClient] ✓ Updated cards display for player at relative index ${playerIndex}, cards:`, result.cards);
            }
        }

        // 从所有玩家手牌中移除已出的牌（重要：这样补牌时才能正确计算 dealCount）
        // 这一步在所有玩家展示完牌之后进行
        log.debug('[TheDecreeModeClient] Removing played cards from all players...');
        for (const result of data.results) {
            const playerIndex = this.getPlayerIndex(result.playerId);
            ...
            if (player) {
                ...
                log.debug(`[TheDecreeModeClient] Removed played cards from player ${result.playerId}: ${oldHandCount} -> ${remainingCards.length}`);
            }
        }
        ...
        log.debug('[TheDecreeModeClient] Sorted results (weak to strong):', sortedResults.map(r => `${r.playerId}: ${r.handTypeName}`));
    ...
    private showHandTypesSequentially(sortedResults: ShowdownResult[], index: number): void {
        if (index >= sortedResults.length) {
            // 所有牌型展示完毕，显示赢家消息
            log.debug('[TheDecreeModeClient] All hand types displayed, showing winner message');
        ...
        log.debug(`[TheDecreeModeClient] Displayed hand type for player ${result.playerId} at index ${playerIndex}: ${scoreText}`);
    ...
    private showWinnerMessageAndCleanup(sortedResults: ShowdownResult[]): void {
        ...
        if (!winner) {
            log.warn('[TheDecreeModeClient] No winner found in results');
            this.clearShowdownDisplay();
            return;
        }
        ...
        // 在消息显示完毕后清除所有牌型显示
        this.showdownClearTimer = setTimeout(() => {
            log.debug('[TheDecreeModeClient] Winner message finished, clearing showdown display');
            this.clearShowdownDisplay();
    ...
    private onRoundEnd(data: RoundEndEvent): void {
        log.debug('[TheDecreeModeClient] ========== Round End Event ==========');
        log.debug('[TheDecreeModeClient] Winner ID:', data.winnerId);
        log.debug('[TheDecreeModeClient] Loser ID:', data.loserId);
        log.debug('[TheDecreeModeClient] Scores:', data.scores);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        ...
        this.cardsToPlay = 0;
        log.debug('[TheDecreeModeClient] Reset cardsToPlay to 0 at round end');
        ...
        const playerUIManager = this.game.playerUIManager;
        if (!playerUIManager) {
            log.error('[TheDecreeModeClient] PlayerUIManager not found, cannot update scores');
            return;
        }
        ...
        if (!currentRoom || !currentRoom.players) {
            log.error('[TheDecreeModeClient] No room data or players found');
            return;
        }

        const players = currentRoom.players;
        log.debug('[TheDecreeModeClient] Updating scores for players:', players.length);
        ...
        log.debug(`[TheDecreeModeClient] My absolute seat: ${myAbsoluteSeat}, total seats: ${totalSeats}`);

        // 遍历所有分数，更新对应玩家的 UI
        for (const player of players) {
            const score = data.scores[player.id];
            if (score !== undefined) {
                log.debug(`[TheDecreeModeClient] Updating score for player ${player.id} (${player.name}): ${score}`);

                // 将绝对座位索引转换为相对座位索引
                const relativeSeat = playerUIManager.getRelativeSeatIndex(player.seatIndex, myAbsoluteSeat, totalSeats);
                log.debug(`[TheDecreeModeClient] Player ${player.name} absolute seat: ${player.seatIndex}, relative seat: ${relativeSeat}`);
                ...
                if (playerUINode) {
                    ...
                    log.debug(`[TheDecreeModeClient] ✓ Score updated for player at relative seat ${relativeSeat}`);
                } else {
                    log.warn(`[TheDecreeModeClient] PlayerUINode not found for relative seat ${relativeSeat}`);
                }
            } else {
                log.warn(`[TheDecreeModeClient] No score found for player ${player.id}`);
            }
        }
    ...
    private onGameOver(data: GameOverEvent): void {
        log.debug('[TheDecreeModeClient] ========== Game Over Event ==========');
        log.debug('[TheDecreeModeClient] Game over:', data);
        log.debug('[TheDecreeModeClient] Winner ID:', data.winnerId);
        log.debug('[TheDecreeModeClient] Total rounds:', data.totalRounds);
        log.debug('[TheDecreeModeClient] Game state:', data.gameState);
        log.debug(`[TheDecreeModeClient] Winner: ${data.winnerId} with ${data.scores[data.winnerId]} points`);
        ...
        if (this._isShowdownInProgress) {
            log.debug('[TheDecreeModeClient] Showdown animation in progress, deferring GameOver processing');
    ...
    private processGameOver(data: GameOverEvent): void {
        log.debug('[TheDecreeModeClient] Processing GameOver event');
        ...
        if (stageManager) {
            ...
            if (playingStage instanceof PlayingStage) {
                log.debug('[TheDecreeModeClient] Notifying PlayingStage that game is finished');
                playingStage.onGameFinished(gameResult);
            } else {
                log.error('[TheDecreeModeClient] PlayingStage not found or onGameFinished method missing!');
            }
        } else {
            log.error('[TheDecreeModeClient] StageManager not found! Cannot switch to EndStage');
        }
    ...
    private prepareGameResult(data: GameOverEvent): any {
        ...
        log.debug('[TheDecreeModeClient] Game result prepared:', rankings);
    ...
    private upgradePlayerUIToPlayingMode(): void {
        log.debug('[TheDecreeModeClient] Upgrading PlayerUIManager to Playing mode...');
        ...
        if (playerUIManager.maxSeats === 0) {
            log.debug('[TheDecreeModeClient] PlayerUIManager not initialized, calling initForReadyStage first...');
            ...
            log.debug('[TheDecreeModeClient] PlayerUIManager initialized for reconnect scenario');
        }

        // 设置玩家 ID 映射（关键！）
        log.debug('[TheDecreeModeClient] Setting up player ID mapping...');
        log.debug('[TheDecreeModeClient] Room players:', currentRoom.players.map(p => ({
            id: p.id,
            name: p.name,
            seatIndex: p.seatIndex
        })));

        log.debug('[TheDecreeModeClient] My player ID:', myPlayerId);
        log.debug('[TheDecreeModeClient] My seat index:', mySeatIndex);
        ...
        log.debug('[TheDecreeModeClient] Remapped players:', remappedPlayers.map(p => ({
            id: p.id,
            name: p.name,
            seatIndex: p.seatIndex,
        ...
        log.debug('[TheDecreeModeClient] Player ID mapping:', Array.from(this.playerIdToIndexMap.entries()));
        ...
        if (!pokerSprites || !pokerPrefab) {
            log.error('[TheDecreeModeClient] Poker resources not loaded!');
            return;
        }
        ...
        log.debug('[TheDecreeModeClient] PlayerUIManager upgraded to Playing mode');
    }
    ...
    private displayCommunityCards(): void {
        log.debug('[TheDecreeModeClient] Displaying community cards:', this.communityCards);

        if (!this.communityCardsNode) {
            log.warn('[TheDecreeModeClient] CommunityCardsNode not found, cannot display cards');
            return;
        }

        if (this.communityCards.length === 0) {
            log.warn('[TheDecreeModeClient] No community cards to display');
            return;
        }
        ...
        log.debug(`[TheDecreeModeClient] Displayed ${this.communityCards.length} community cards`);
    }
    ...
    private displaySingleCommunityCard(index: number, cardValue: number, showFront: boolean = true): void {
        ...
        const pokerFactory = PokerFactory.instance;
        if (!pokerFactory) {
            log.error('[TheDecreeModeClient] PokerFactory instance not found!');
            return;
        }
        ...
        if (!pokerPrefab) {
            log.error('[TheDecreeModeClient] Poker prefab not found!');
            return;
        }
        ...
        if (pokerFront && pokerBack) {
            ...
            log.warn(`[TheDecreeModeClient] Sprite not found: ${spriteName}`);
        }
        ...
        log.debug(`[TheDecreeModeClient] Created community card ${index + 1}: ${spriteName} (${showFront ? 'front' : 'back'})`);
    }
    ...
    private flipAllCommunityCards(): void {
        ...
        log.debug(`[TheDecreeModeClient] Flipping ${children.length} community cards`);
    ...
    private clearShowdownTimer(): void {
        if (this.showdownClearTimer !== null) {
            ...
            log.debug('[TheDecreeModeClient] Showdown clear timer cancelled');
        }
    }
    ...
    private clearShowdownDisplay(): void {
        log.debug('[TheDecreeModeClient] Clearing showdown display...');
        ...
        log.debug('[TheDecreeModeClient] ✓ All hand types cleared');
        ...
        for (let i = 1; i < playerCount; i++) {
            ...
            if (playerUINode) {
                ...
                if (handDisplay) {
                    // 清除出牌显示，恢复为手牌背面
                    handDisplay.updateDisplay([]);
                    log.debug(`[TheDecreeModeClient] Cleared showdown display for player at index ${i}`);
                }
            }
        }

        log.debug('[TheDecreeModeClient] ✓ Showdown display cleared for all players');
    ...
    private onShowdownAnimationComplete(): void {
        log.debug('[TheDecreeModeClient] Showdown animation completed');
        this._isShowdownInProgress = false;

        // 检查是否有待处理的 GameOver 事件
        if (this._pendingGameOver) {
            log.debug('[TheDecreeModeClient] Processing pending GameOver event');
    ...
    public initGame(playerInfos: PlayerInfo[]): void {
        log.debug('[TheDecreeModeClient] initGame called with', playerInfos.length, 'players');

        // 设置玩家 ID 映射
        this.refreshPlayerIdMapping();
        if (this.playerIdToIndexMap.size === 0) {
            log.warn('[TheDecreeModeClient] Player mapping not ready, falling back to absolute seatIndex mapping');
    ...
    public dealCards(): void {
        log.debug('[TheDecreeModeClient] dealCards called - waiting for server');
    ...
    public playCards(cards: number[], playerId: string): boolean {
        log.debug('[TheDecreeModeClient] playCards called - sending to server');
    ...
    public dealerCall(cardsToPlay: 1 | 2 | 3): boolean {
        log.debug('[TheDecreeModeClient] Dealer calling', cardsToPlay, 'cards');

        // 使用基类的发送方法
        const success = this.sendDealerCallRequest(cardsToPlay);

        if (success) {
            log.debug('[TheDecreeModeClient] ✓ Dealer call request sent successfully');
        } else {
            log.error('[TheDecreeModeClient] ✗ Failed to send dealer call request');
        }
    ...
    public setPlayer0AutoPlay(enabled: boolean): void {
        ...
        log.debug(`[TheDecreeModeClient] Player_0 auto-play ${enabled ? 'enabled' : 'disabled'}`);
    }
    ...
    public getPlayer(playerId: string): any | undefined {
        log.warn('[TheDecreeModeClient] getPlayer() not supported in network mode');
        return undefined;
    }
    ...
    public getScores(): Map<string, number> {
        log.warn('[TheDecreeModeClient] getScores() not supported in network mode');
        return new Map();
    }
    ...
    public getCurrentRound(): any | null {
        log.warn('[TheDecreeModeClient] getCurrentRound() not supported in network mode');
        return null;
    }
    ...
    public selectFirstDealer(revealedCards: Map<string, number>): string {
        log.warn('[TheDecreeModeClient] selectFirstDealer() not supported in network mode');
        return '';
    }
    ...
    public startNewRound(dealerId: string): void {
        log.warn('[TheDecreeModeClient] startNewRound() not supported in network mode - controlled by server');
    }
    ...
    public refillHands(): void {
        log.warn('[TheDecreeModeClient] refillHands() not supported in network mode - controlled by server');
    }
    ...
    private onPlayerAutoChanged(data: PlayerAutoChangedEvent): void {
        log.debug('[TheDecreeModeClient] ========== Player Auto Changed Event ==========');
        log.debug('[TheDecreeModeClient] Player ID:', data.playerId);
        log.debug('[TheDecreeModeClient] Is Auto:', data.isAuto);
        log.debug('[TheDecreeModeClient] Reason:', data.reason);
        ...
        log.debug('[TheDecreeModeClient] ✓ Player auto state updated');
        log.debug('[TheDecreeModeClient] ========================================');
    }
    ...
    public setAuto(isAuto: boolean): void {
        ...
        if (!networkClient) {
            log.error('[TheDecreeModeClient] Network client not available');
            return;
        }

        networkClient.send(ClientMessageType.SET_AUTO, request);

        log.debug(`[TheDecreeModeClient] Set auto mode: ${isAuto}`);
    }
    ...
    private checkAndRestoreReconnectState(force: boolean = false): void {
        log.debug('[TheDecreeModeClient] checkAndRestoreReconnectState, force:', force);
        log.debug('[TheDecreeModeClient] game instance:', {
            node: this.game?.node?.name,
            uuid: this.game?.node?.uuid,
            scene: this.game?.node?.scene?.name,
        ...
        try {
            ...
            log.debug('[TheDecreeModeClient] reconnect snapshot:', {
            ...
            if (!force && communityCards.length === 0 && myHandCards.length === 0) {
                log.debug('[TheDecreeModeClient] No reconnect data found, waiting for server events');
                return;
            }

            log.debug('[TheDecreeModeClient] ========== Restoring Reconnect State ==========');
            log.debug('[TheDecreeModeClient] Community cards:', communityCards);
            log.debug('[TheDecreeModeClient] My hand cards:', myHandCards.length);
            ...
            if (savedGameState) {
                ...
                log.debug('[TheDecreeModeClient] Restored game state:', this.gameState);
            }
            ...
            if (savedDealerId) {
                ...
                log.debug('[TheDecreeModeClient] Restored dealer ID:', this.dealerId);
            ...
            if (savedCardsToPlay > 0) {
                ...
                log.debug('[TheDecreeModeClient] Restored cards to play:', this.cardsToPlay);
            }
            ...
            if (savedRound > 0) {
                ...
                log.debug('[TheDecreeModeClient] Restored round number:', this.currentRoundNumber);
            }
            ...
            if (communityCards.length > 0) {
                ...
                this.displayCommunityCards();
                log.debug('[TheDecreeModeClient] Restored community cards display');
            }
            ...
            if (myHandCards.length > 0) {
                ...
                log.debug('[TheDecreeModeClient] Restored hand cards display');
            }
            if (myHandCards.length === 0) {
                this.restoreHandCardsDisplay(myHandCards);
                log.debug('[TheDecreeModeClient] Restored hand cards display (empty hand)');
            }
            ...
            if (this.theDecreeUIController) {
                ...
                if (this.gameState === TheDecreeGameState.PLAYER_SELECTION && this.cardsToPlay > 0) {
                    ...
                    log.debug('[TheDecreeModeClient] Restored cards-to-play hint on reconnect');
                }
            }

            log.debug('[TheDecreeModeClient] ========== Reconnect State Restored ==========');
        } catch (error) {
            log.error('[TheDecreeModeClient] Failed to restore reconnect state:', error);
        } finally {
            log.debug('[TheDecreeModeClient] finishing reconnect loading');
    ...
    private restoreHandCardsDisplay(myHandCards: number[]): void {
        ...
        if (playerUIController) {
            ...
            if (player) {
                ...
                playerUIManager.updatePlayerHand(0);
                log.debug('[TheDecreeModeClient] Main player hand restored:', myHandCards.length, 'cards');
        ...
        for (const playerData of allPlayerData) {
            ...
            if (otherPlayerUIController) {
                ...
                if (otherPlayer) {
                    ...
                    otherPlayer.setHandCards(emptyCards);
                    playerUIManager.updatePlayerHand(playerIndex);
                    log.debug(`[TheDecreeModeClient] Player ${playerIndex} hand count restored: ${playerData.cardCount}`);
                }
            }
        }
    ...
    private restoreScoresDisplay(): void {
        ...
        for (const [playerId, score] of allScores) {
            ...
            if (playerUINode) {
                playerUINode.updateScore(score);
                log.debug(`[TheDecreeModeClient] Restored score for player ${playerIndex}: ${score}`);
            }
        }
    }
    ...
    private refreshPlayerIdMapping(): void {
        ...
        const currentRoom = localRoomStore.getCurrentRoom();
        if (!currentRoom) {
            log.warn('[TheDecreeModeClient] No current room for player mapping');
            return;
        }
        ...
        log.debug('[TheDecreeModeClient] Player ID mapping refreshed:', Array.from(this.playerIdToIndexMap.entries()));
    }
}
